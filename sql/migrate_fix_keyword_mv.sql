-- ============================================================
-- 修正マイグレーション: キーワード分析ビュー・RPC を MV 参照に変更
-- + topic_ids GIN インデックス追加
-- 問題: migrate_keyword_analysis.sql が DISTINCT ON を直接使用していた
--       → video_snapshots フルスキャンによる statement timeout
-- ============================================================

-- 1. GIN インデックス追加（ANY(topic_ids) パターン高速化）
...
-- ============================================================
-- 4. fn_keyword_opportunity RPC: DISTINCT ON → MV 参照
-- ============================================================
...
-- ============================================================
-- 5. keyword_virality ビュー: DISTINCT ON → MV 参照
-- ============================================================
DROP VIEW IF EXISTS keyword_virality CASCADE;
CREATE VIEW keyword_virality AS
WITH tag_buzz AS (
    SELECT
        LOWER(TRIM(tag)) AS tag,
...
-- ============================================================
-- 6. fn_keyword_virality RPC: DISTINCT ON → MV 参照
-- ============================================================
