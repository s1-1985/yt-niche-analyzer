-- ============================================================
-- 時系列フィルタ対応: RPC関数（ビューと同等だが期間制限付き）
-- p_min_date = NULL の場合は全期間
-- ============================================================

-- ジャンル別サマリー（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_topic_summary(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
...
BEGIN
    ...
    WITH topic_videos AS (
        SELECT
            ...
            v.id AS vid,
            v.channel_id AS vchannel,
            vs.view_count AS vview,
            vs.like_count AS vlike,
            vs.comment_count AS vcomment,
            vs.snapshot_date AS sdate
        FROM topics t
    ...
    latest AS (
        SELECT DISTINCT ON (tid, vid) *
        FROM topic_videos
        ORDER BY tid, vid, sdate DESC
    )
    SELECT
        l.tid,
        l.tname,
        ...
        l.tparent,
        l.tcategory,
        COUNT(DISTINCT l.vid),
        COUNT(DISTINCT l.vchannel),
        COALESCE(SUM(l.vview), 0),
        COALESCE(AVG(l.vview), 0)::BIGINT,
        CASE
            WHEN COUNT(DISTINCT l.vchannel) > 0
            THEN (COALESCE(AVG(l.vview), 0) / COUNT(DISTINCT l.vchannel))::BIGINT
            ELSE 0
        END,
        CASE
            WHEN COALESCE(SUM(l.vview), 0) > 0
            THEN ROUND(COALESCE(SUM(l.vlike), 0)::NUMERIC / SUM(l.vview) * 100, 2)
            ELSE 0
        END,
        CASE
            WHEN COALESCE(SUM(l.vview), 0) > 0
            THEN ROUND(COALESCE(SUM(l.vcomment), 0)::NUMERIC / SUM(l.vview) * 100, 4)
            ELSE 0
        END
    FROM latest l
...
-- 競合集中度（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_competition_concentration(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
    topic_total_views BIGINT,
    top5_views BIGINT,
...
BEGIN
    ...
    ranked AS (
        SELECT *,
            ROW_NUMBER() OVER (PARTITION BY tid ORDER BY total_v DESC) AS rank,
    ...
    SELECT
        ...
        r.tname,
        r.tname_ja,
        r.topic_total::BIGINT,
        SUM(r.total_v) FILTER (WHERE r.rank <= 5)::BIGINT,
        ROUND(
            SUM(r.total_v) FILTER (WHERE r.rank <= 5)::NUMERIC / NULLIF(r.topic_total, 0) * 100, 1
        )
    FROM ranked r
...
-- 新規チャンネル成功率（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_new_channel_success_rate(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
    new_channel_count BIGINT,
...
BEGIN
    ...
    WITH new_channels AS (
        SELECT
            c.id,
            c.topic_ids AS ctopic_ids,
            cs.subscriber_count AS csub
        FROM channels c
        JOIN (
    ...
    SELECT
        ...
        ROUND(
            COUNT(*) FILTER (WHERE nc.csub >= 1000)::NUMERIC / NULLIF(COUNT(*), 0) * 100, 1
        )
    FROM topics t
...
-- AI浸透度（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_ai_penetration(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
    total_videos BIGINT,
    ai_video_count BIGINT,
    ai_penetration_pct NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ...
        ROUND(
            COUNT(*) FILTER (WHERE v.has_ai_keywords = TRUE)::NUMERIC / NULLIF(COUNT(*), 0) * 100, 2
        )
    FROM topics t
    JOIN videos v ON t.id = ANY(v.topic_ids)
    WHERE (p_min_date IS NULL OR v.published_at >= p_min_date)
...
-- 動画尺統計（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_topic_duration_stats(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
    ...
    p25_duration INTEGER,
    p75_duration INTEGER,
    short_count BIGINT,
    medium_count BIGINT,
    long_count BIGINT
) AS $$
BEGIN
    ...
    WITH topic_videos AS (
        SELECT
            t.id AS tid, t.name AS tname, t.name_ja AS tname_ja, t.parent_id AS tparent,
...
-- チャンネル規模分布（フィルタ付き: 期間内に動画があるチャンネルのみ）
CREATE OR REPLACE FUNCTION fn_topic_channel_size(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
...
BEGIN
    ...
    latest_subs AS (
        ...
        JOIN active_channels ac ON cs.channel_id = ac.channel_id
    ...
    SELECT
        tc.tid, tc.tname, tc.tname_ja, tc.tparent,
...
-- 投稿曜日分析（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_topic_publish_day(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
...
BEGIN
    ...
    SELECT
        tv.tid, tv.tname, tv.tname_ja, tv.tparent, tv.vdow,
        COUNT(*)::BIGINT,
...
-- 国別チャンネル分布（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_topic_country_distribution(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
...
BEGIN
    ...
    SELECT
        t.id, t.name, t.name_ja, t.parent_id,
...
-- 人気タグ TOP10（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_topic_popular_tags(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_id TEXT,
    topic_name TEXT,
    name_ja TEXT,
...
BEGIN
    ...
    WITH tag_data AS (
        SELECT
            t.id AS tid, t.name AS tname, t.name_ja AS tname_ja,
...
-- チャンネル成長効率（フィルタ付き: 期間内に動画があるチャンネルのみ）
CREATE OR REPLACE FUNCTION fn_channel_growth_efficiency(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    channel_id TEXT,
    title TEXT,
    ...
    video_count INTEGER,
    age_days INTEGER,
    subs_per_day NUMERIC,
    views_per_video NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    WITH active_channels AS (
    ...
    SELECT
        c.id, c.title, c.published_at, c.country, c.topic_ids,
...
-- ジャンル相関（フィルタ付き）
CREATE OR REPLACE FUNCTION fn_topic_overlap(p_min_date TIMESTAMPTZ DEFAULT NULL)
RETURNS TABLE(
    topic_a TEXT,
    name_a TEXT,
    topic_b TEXT,
    name_b TEXT,
    shared_channels BIGINT
) AS $$
BEGIN
    RETURN QUERY
    WITH active_channels AS (
    ...
    SELECT
        t1.id, t1.name_ja, t2.id, t2.name_ja,
        COUNT(DISTINCT c.id)::BIGINT
    FROM channels c
