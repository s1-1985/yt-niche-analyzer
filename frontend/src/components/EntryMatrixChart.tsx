...
export function EntryMatrixChart({ topics, competition, onTopicClick }: Props) {
  ...
  const chartData: ChartEntry[] = subTopics
    ...
    .map((t) => {
      const top5 = compMap.get(t.topic_id) ?? 0;
      return {
        ...
        topic_id: t.topic_id,
        category: t.category,
        entry_score: Math.round(t.gap_score * (100 - top5) / 100),
      };
    });

  const gapMedian = chartData.length > 0
    ? [...chartData].sort((a, b) => a.gap_score - b.gap_score)[Math.floor(chartData.length / 2)].gap_score : 0;
  const compMedian = chartData.length > 0
    ? [...chartData].sort((a, b) => a.top5_share - b.top5_share)[Math.floor(chartData.length / 2)].top5_share : 50;
  ...
  const rankingItems: RankingItem[] = [...chartData]
    ...
    .map((d) => ({
      name: d.name,
      value: d.entry_score,
      sub: `Gap:${d.gap_score.toLocaleString()} / 集中:${d.top5_share}%`,
      topic_id: d.topic_id,
    }));
  ...
  return (
    <div className="chart-card">
      <div className="chart-title-row">
        <h3>参入難易度マトリクス</h3>
        <HelpButton {...HELP_TEXTS.entryMatrix} />
      </div>
      <p className="chart-desc">
        右下が狙い目ゾーン（需要が高く、競合が分散）。左上は避けるべきゾーン
      </p>
      ...
      {showList ? (
        ...
        <>
          <div className="chart-axis-labels">
            <span>X: 需給ギャップ（右=需要大）</span>
            <span>Y: 競合集中度（下=参入しやすい）</span>
          </div>
          <ResponsiveContainer width="100%" height={isMobile ? 300 : 450}>
            <ScatterChart margin={isMobile
              ...
              <XAxis type="number" dataKey="gap_score" name="需給ギャップ" tick={{ fontSize: isMobile ? 9 : 11 }}>
                {!isMobile && <Label value="需給ギャップ (高い = 需要大)" position="bottom" offset={10} style={{ fill: '#9ca3af', fontSize: 12 }} />}
              </XAxis>
              <YAxis type="number" dataKey="top5_share" name="Top5集中度" unit="%" domain={[0, 100]}
                tick={{ fontSize: isMobile ? 9 : 11 }} width={isMobile ? 35 : 60}>
                {!isMobile && <Label value="競合集中度 (低い = 参入しやすい)" angle={-90} position="insideLeft" offset={-5} style={{ fill: '#9ca3af', fontSize: 12 }} />}
              </YAxis>
              <ZAxis type="number" dataKey="avg_views" range={isMobile ? [30, 200] : [60, 400]} name="平均再生" />
              <ReferenceArea x1={gapMedian} x2={maxGap * 1.1} y1={0} y2={compMedian}
                fill="#10b981" fillOpacity={0.08} stroke="#10b981" strokeOpacity={0.2} strokeDasharray="4 4" />
              <ReferenceArea x1={0} x2={gapMedian} y1={compMedian} y2={maxComp * 1.05}
                fill="#ef4444" fillOpacity={0.06} stroke="#ef4444" strokeOpacity={0.15} strokeDasharray="4 4" />
              ...
              <Scatter data={chartData} cursor="pointer"
                ...
                {chartData.map((entry, index) => (
                  <Cell key={index} fill={CATEGORY_COLORS[entry.category] ?? '#6366f1'}
                    stroke="#ffffff" strokeWidth={isMobile ? 1 : 2} />
                ))}
              </Scatter>
            </ScatterChart>
          </ResponsiveContainer>
